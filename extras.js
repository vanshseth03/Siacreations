...
(function(){
        ...
        const html = window.wishlist.map(item => {
            ...
            return `
                <div class="wishlist-item" data-id="${item.id}">
                    <div class="wishlist-item-image" onclick="window.location.href='item-view.html?id=${item.id}'">${item.image}</div>
                    <div class="wishlist-item-info">
                        <div class="wishlist-item-name">${item.name}</div>
                        <div class="wishlist-item-price">₹${item.price}</div>
                        <div class="wishlist-item-controls">
                            <button class="add-to-cart" onclick="event.stopPropagation(); if(typeof addToCart === 'function') addToCart(${item.id})">Add to Cart</button>
                            <button class="remove-btn" onclick="event.stopPropagation(); if(typeof removeFromWishlist === 'function') removeFromWishlist(${item.id}); renderWishlistSidebar()">✕</button>
                        </div>
                    </div>
                </div>
            `;
    ...
    document.addEventListener('DOMContentLoaded', function(){
        try { setupLinks(); } catch(e) { console.error('setupLinks error:', e); }
        try { 
